...
<body>
	...
	$array = qst_get_question ( $id, $con );
	$question = $array [0];
	?>
<div class="container">
		<form class="form-horizontal" id="form-add-question" method="post"
			action="../common/question_edit_status.php">
				...
				<div class="col-md-6">
					<div class="radio">
						<label> <input type="radio" name="type" id="type-choice"
							value="选择题" <?php echo $question["type"]=='选择题'?'checked':''; ?>>选择题
						</label>
					</div>
					<div class="radio">
						<label> <input type="radio" name="type" id="type-blank"
							value="填空题" <?php echo $question["type"]=='填空题'?'checked':''; ?>>填空题
						</label>
					</div>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-md-6">
					<textarea class="form-control" name="content" rows="3"><?php echo trim($question["content"]); ?></textarea>
				</div>
			</div>
			<div class="form-group">
			...
			<?php
			// 获取该题目所有选项，填充在这里
			$sql = "select option_id,content from options where question_id=? order by option_id";
			...
			foreach ( $array as $option ) {
				...
				echo '<span class="input-group-addon">' . $option ["option_id"] . '</span>';
				echo '<input type="text" class="form-control" name="option-' . $option ["option_id"] . '" value="' . $option ["content"] . '" />';
				echo '</div>';
				echo '</div>';
				echo '</div>';
			}
